...
<form id="form-id" method="POST">
  ...
  <!--Account Setup-->
  <fieldset>
    <h2 class='title'>Update your Appoiontment  </h2>
...
if(isset($_POST["appp"]))
{
  ...
  $conn = mysqli_connect("localhost","root","","mankind") or die ("DataBase Error");
    ...
    $sql = "UPDATE `appoinment` SET `Name`='$nm',`Email`='$em',`Date`='$dt',`Time`='$tm',`Phone`='$ph',`Location`='$loc',`Description`='$des' WHERE `Name`= '".$abc."'";

    if(mysqli_query($conn,$sql))
    {
        echo '  <script>
        alert("Update Successful "); 
        window.location.replace("viewappoin.php");  
