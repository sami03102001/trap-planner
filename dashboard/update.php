...
<form id="form-id" method="POST">
  ...
  <fieldset>
    ...
    <input type='text' name='fn' placeholder='First Name' value="<?php echo $_SESSION['ufn'];?>">
    <input type='text' name='ln' placeholder='Last Name' value="<?php echo $_SESSION['uln'];?>">
    <input type='button' name='next' class='next-button custom-button' value="Next">
  </fieldset>
  <!--Social Profiles-->
  <fieldset>
    <h2 class='title'>Update your Account</h2>
    <input type='email' name='em' placeholder='Email' value="<?php echo $_SESSION['uem'];?>">
    <input type='text' name='pass' placeholder='Password' value="<?php echo $_SESSION['upass'];?>">
...
<fieldset>
    ...
    <input type='text' name='con' placeholder='Contact' value="<?php echo $_SESSION['ucon'];?>">
    <input type='text' name='age' placeholder='Age' value="<?php echo $_SESSION['uage'];?>">
    <input type='button' name='previous' class='prev-button custom-button' value="Back">
    <button name="finish" class='custom-button' >Finish</button>
  </fieldset>
...
if(isset($_POST["finish"]))
{
  ...
  $conn = mysqli_connect("localhost","root","","mankind") or die ("DataBase Error");
    ...
    $sql = "UPDATE `users` SET `FirstName`='".$fn."',`LastName`='".$ln."',`Email`='".$em."',`Pasword`='".$pass."',`Contact`='".$con."',`Age`='".$age."' WHERE `Email`= '".$em."'";

    if(mysqli_query($conn,$sql))
    {
        echo '  <script>
        alert("Update Successful (Login Again)"); 
        window.location.replace("../logout.php");  
